...
(slide #:title "PowerShell 101"
   ...
   (item #:bullet (blank) (h (s-t "> " font-size-in-powershell-101)
                             ...
                             (s-t "Jenkins" font-size-in-powershell-101)))
   'next
   (item #:bullet (blank) (h (s-t "> " font-size-in-powershell-101)
...
(slide #:title "Define a Task"
   (v (* gap-size 2)
       (frame
        ...
        (ps "    <Exec Cmd=\"$Yam build YOUR_PROJ\">")
        (ps "</Target>")
        ) 20))
      (frame
       (inset (v
...
(define (task-invocation show-func-name show-first-arg show-second-arg show-third-arg)
  (slide #:title "Task Invocation"
      (let* ([task-invocation-font-size 28]
             ...
             [second-arg (h (s-ps " -depends " task-invocation-font-size) (s-highlight "UnitTest" task-invocation-font-size))]
...
(slide #:title "More sophisticated parameter mechanism"
  ...
  (v
   ...
   (h (s-highlight "    [Parameter(Mandatory=$true)]" font-size) (s-ps " $name=$null," font-size))
   (h (s-highlight "    [Parameter(Mandatory=$false)]" font-size)(s-ps " $action=$null," font-size))
   (h (s-highlight "    [Parameter(Mandatory=$false)]" font-size) (s-ps " $depends=@()" font-size))
   (s-ps "  )" font-size)
...
(slide #:title "More sophisticated parameter mechanism"
  ...
  (v
   (s-ps "Function Task {" font-size)
   (s-ps "  param(" font-size)
   (h (s-ps "    [Parameter(Mandatory=$true)]" font-size) (s-highlight "[string]" font-size) (s-ps " $name=$null," font-size))
   (h (s-ps "    [Parameter(Mandatory=$false)]" font-size) (s-highlight "[scriptblock]" font-size) (s-ps " $action=$null," font-size))
   (h (s-ps "    [Parameter(Mandatory=$false)]" font-size) (s-highlight "[string[]]" font-size) (s-ps " $depends=@()" font-size))
   (s-ps "  )" font-size)
   (s-ps "  # Invoke task ..." font-size)
   (s-ps "}" font-size))))
